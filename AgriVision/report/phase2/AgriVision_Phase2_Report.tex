...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{dl_results.png}
    \caption{Left: Training and validation MSE loss per epoch.
             Right: Predicted vs.\ actual yield (t/ha) on the test set.
             Val loss tracks train throughout with a small gap (no overfitting).
             The visible kink marks the first \texttt{ReduceLROnPlateau} halving
             event. Points cluster tightly around the diagonal ($R^2 = 0.929$).}
    \label{fig:learning_curves}
\end{figure}

The learning curves (Fig.~\ref{fig:learning_curves}) show:
\begin{itemize}
    ...
    \item \textbf{Predicted vs.\ actual}: the scatter plot shows test-set predictions
          aligned closely with the identity line, confirming $R^2 = 0.929$.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{dl_per_crop_r2.png}
    \caption{Per-crop $R^2$ on the held-out test set.
             The dashed red line marks the overall $R^2 = 0.929$.
             Red bars (if any) indicate crops where the model underperforms
             the mean predictor; all bars positive confirms generalisation
             across all 10 crop types.}
    \label{fig:per_crop_r2}
\end{figure}

Per-crop $R^2$ is computed by isolating test-set predictions for each crop
(Fig.~\ref{fig:per_crop_r2}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{dl_attention_weights.png}
    \caption{Attention weight distributions $(\alpha_1,\ldots,\alpha_5)$ for
             8 sample test sequences.
             Each bar represents the fraction of model attention allocated to
             one lookback year ($t{-}4$ through $t$).
             Taller bars toward $t$ indicate recency bias; shifts toward
             earlier timesteps reveal multi-year cumulative stress signals.}
    \label{fig:attention}
\end{figure}

Attention weights $\alpha_1, \ldots, \alpha_5$ are extracted for test sequences and
visualised as bar charts in Fig.~\ref{fig:attention}.
